...
extension StickersService: KlipyTargetType {
  public var path: String {
    switch self {
    case .trending:
      return PathComponent.stickers + PathComponent.trending
      
    case .search:
      return PathComponent.stickers + PathComponent.search
      
    case .categories:
      return PathComponent.stickers + PathComponent.categories
      
    case .recent(let customerId, _, _):
      return PathComponent.stickers + PathComponent.recent + "/\(customerId)"
      
    case .hideFromRecent(let customerId, _):
      return PathComponent.stickers + PathComponent.recent + "/\(customerId)"
      
    case .view(let slug, _):
      return PathComponent.stickers + PathComponent.view + "/\(slug)"
      
    case .share(let slug, _):
      return PathComponent.stickers + PathComponent.share + "/\(slug)"
      
    case .report(let slug, _, _):
      return PathComponent.stickers + PathComponent.report + "/\(slug)"
    }
  }
